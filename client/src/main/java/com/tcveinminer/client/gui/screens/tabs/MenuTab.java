package com.tcveinminer.client.gui.screens.tabs;

import com.tcveinminer.client.gui.screens.MainMenuScreen;
import net.minecraft.client.gui.GuiGraphics;

public interface MenuTab {
    void init(MainMenuScreen screen, int cx, int cy, int cw, int ch);
    void render(GuiGraphics ctx, MainMenuScreen screen, int cx, int cy, int cw, int ch, int mouseX, int mouseY, float delta);
    default boolean mouseClicked(MainMenuScreen screen, double mx, double my, int btn) { return false; }
    default boolean mouseScrolled(MainMenuScreen screen, double mx, double my, double h, double v) { return false; }
    default boolean mouseDragged(MainMenuScreen screen, double mx, double my, int btn, double dx, double dy) { return false; }
    default boolean mouseReleased(MainMenuScreen screen, double mx, double my, int btn) { return false; }
}

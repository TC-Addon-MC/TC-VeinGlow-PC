...
public class ProtectThresholdSlider extends AbstractSliderButton {
    private final MenuState state;

    public ProtectThresholdSlider(int x, int y, int w, int h, MenuState state) {
        super(x, y, w, h, Component.literal(String.valueOf(state.toolProtectThreshold)),
                state.toolProtectThreshold / 100.0);
        this.state = state;
    }
    ...
    protected void updateMessage() {
        ...
        if (state.toolProtectThreshold < 1)
            state.toolProtectThreshold = 1;

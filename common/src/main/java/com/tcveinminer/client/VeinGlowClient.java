...
public final class VeinGlowClient {
    ...
    public static void cycleShape(MinecraftClient client, int direction) {
    ...
    public static void selectShapeByIndex(MinecraftClient client, int index) {
        var enabledShapes = new ArrayList<>(ClientConfigManager.instance.enabledShapes);
    ...
    public static void handleMiningState(MiningStateData data) {
        ...
        isMining = (state == 1);
        com.tcveinminer.client.logic.HudNotifier.lastMined = data.broken();
        com.tcveinminer.client.logic.HudNotifier.lastMax = data.target();
        if (state == 2 || state == 3) {
            com.tcveinminer.client.logic.HudNotifier.notifyAt = System.currentTimeMillis() + 2500;
            com.tcveinminer.client.logic.HudNotifier.lastCancelled = (state == 3);
        }
    }
    ...
    public static void handleLookedAtBlock(LookedAtBlockData data) {
        BlockHighlighter.lookedAtBlock = data.pos().orElse(null);
    }
    ...
    public static void handleHighlightDelta(HighlightDeltaData data) {
        BlockHighlighter.highlightBlocks.removeAll(data.removedBlocks());
        BlockHighlighter.highlightBlocks.addAll(data.addedBlocks());
    ...
    public static void handleHighlightBlockList(HighlightBlockListData data) {
        ...
        BlockHighlighter.highlightBlocks.addAll(data.blocks());
        BlockHighlighter.highlightStyle = data.highlightStyle();
    }

    public static void onClientTick(MinecraftClient client, boolean keyMinePressed, boolean menuKeyPressed) {
        ...
        if (client.world != null) {
            ...
            if (forceSend || (targetChanged && now - lastActivationSendTime > 100)) {
                ...
                if (ClientNetworkManager.canSend(ActivationRequestData.class)) {
                    ClientNetworkManager.sendToServer(new ActivationRequestData(holdKeyDown, java.util.Optional.ofNullable(currentTarget)));
                }
            }
        }
    }

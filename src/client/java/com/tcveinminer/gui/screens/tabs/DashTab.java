...
public class DashTab implements MenuTab {
    ...
    public void render(DrawContext ctx, MainMenuScreen screen, int cx, int cy, int cw, int ch, int mouseX, int mouseY, float delta) {
        ...
        ctx.drawTextWithShadow(screen.getTextRenderer(), Text.translatable("gui.tcveinminer.dash.status_title").getString(), cx + 12, cy + 10, 0xFFA0AEC0);
        ...
        if (innerH > 15) {
            ...
            ctx.drawTextWithShadow(screen.getTextRenderer(), Text.translatable("gui.tcveinminer.dash.shape_mode").getString(), cx + 20, innerY + 6, 0xFF6B7280);
            String label = screen.getState().selectedShapeId;
            try { label = Text.translatable("tc_veinminer.mode." + screen.getState().selectedShapeId).getString(); } catch (Exception ignored) {}
            ...
            ctx.drawTextWithShadow(screen.getTextRenderer(), Text.translatable("gui.tcveinminer.dash.activation_mode").getString(), rightX + 8, innerY + 6, 0xFF6B7280);
        ...
        ctx.drawTextWithShadow(screen.getTextRenderer(), Text.translatable("gui.tcveinminer.dash.max_blocks_title").getString(), cx + 12, box2Y + 10, 0xFFFFFFFF);

        String lim = screen.getState().maxBlocks + " / " + ClientConfigManager.instance.serverMaxBlocks + " " + Text.translatable("gui.tcveinminer.dash.blocks_unit").getString();
        ...
        String footerNote = Text.translatable("gui.tcveinminer.dash.max_blocks_desc").getString();

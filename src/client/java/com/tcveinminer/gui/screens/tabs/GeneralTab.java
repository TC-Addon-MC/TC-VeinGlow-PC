...
public class GeneralTab implements MenuTab {
    ...
    public void init(MainMenuScreen screen, int cx, int cy, int cw, int ch) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            int ry = cy + 18 + (i / 2) * 24;
            screen.addUIElement(new CustomButton(rx, ry, itemW, 20, Text.empty(), btn -> { screen.getState().activationMode = m; screen.rebuildMenu(); }));
        }
        int dY = cy + 70;
        screen.addUIElement(new CustomButton(cx, dY, cw, 20, Text.empty(), btn -> { screen.getState().showHud = !screen.getState().showHud; screen.rebuildMenu(); }));
        screen.addUIElement(new CustomButton(cx, dY + 24, cw, 20, Text.empty(), btn -> { screen.getState().showOutline = !screen.getState().showOutline; screen.rebuildMenu(); }));
    }
    ...
    private void drawCheckRow(DrawContext ctx, MainMenuScreen screen, int rx, int ry, int rw, String label, boolean checked) {
        ctx.fill(rx, ry, rx + rw, ry + 20, ThemeColors.BG_ROW);
        DrawHelper.drawSolidBorder(ctx, rx, ry, rw, 20, ThemeColors.BORDER_DEFAULT);
        ctx.drawTextWithShadow(screen.getTextRenderer(), label, rx + 8, ry + 6, ThemeColors.TEXT_BRIGHT);

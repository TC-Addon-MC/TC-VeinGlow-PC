...
/**
 * BlockHighlighter: render outline xung quanh các block trong mining queue.
 *
 * FIX QUAN TRỌNG: Renderer KHÔNG scan riêng nữa.
 * - Khi engine đang MINING: dùng renderSnapshot từ MiningEngine (server-authoritative)
 * - Khi engine IDLE + player đang hold key: preview scan client-side (chưa commit)
 *
 * Highlighted blocks == queued blocks == sẽ được đào.
 * Nếu block bị remove khỏi queue, nó tự động biến mất khỏi overlay.
 */
public class BlockHighlighter {
    ...
    private static boolean onDrawOutline(WorldRenderContext context, WorldRenderContext.BlockOutlineContext outlineContext) {
        ...
        Set<BlockPos> blocksToHighlight = resolveHighlightSet(client);
        if (blocksToHighlight.isEmpty()) return true;

        drawOutlines(context, client, blocksToHighlight);
        return false;
    }

    /**
     * Resolve highlight set:
     * 1. Nếu engine đang MINING → dùng server queue snapshot (synchronized)
     * 2. Nếu idle + hold key → client preview scan (không commit, chỉ để xem trước)
     */
    private static Set<BlockPos> resolveHighlightSet(MinecraftClient client) {
        // Client preview scan cho trường hợp idle (chưa có queue)
        HitResult hit = client.crosshairTarget;
        if (hit == null || hit.getType() != HitResult.Type.BLOCK) return Collections.emptySet();

        BlockPos targetPos = ((BlockHitResult) hit).getBlockPos();
        BlockState targetState = client.world.getBlockState(targetPos);

        // Preview: gọi strategy trực tiếp (client-side, không modify queue)
        ...
        Set<BlockPos> result = new HashSet<>(preview);
